...
fn dispatch_frames(bus: &WifiBus, buf: &[u8]) {
    ...
    while offset + 4 <= buf.len() {
        ...
        if !is_cfg {
            ...
            // Ethernet 帧从 data_payload[60] 开始  
            // ethertype 在 data_payload[60+12..60+14] = data_payload[72..74]  
            const ETH_HDR_OFFSET: usize = 60; // = RX_HWHRD_LEN = sizeof(hw_rxhdr) + padding  
            const ETH_P_PAE: u16 = 0x888E;  

            if data_payload.len() >= ETH_HDR_OFFSET + 14 {
                let ethertype = u16::from_be_bytes([  
                    data_payload[ETH_HDR_OFFSET + 12],  
                    data_payload[ETH_HDR_OFFSET + 13],  
                ]);  
                if ethertype == ETH_P_PAE {  
                    // EAPOL 帧：提取 EAPOL payload（跳过 Ethernet 头 14 字节） 
                    let eapol_start = ETH_HDR_OFFSET + 14;  
                    if data_payload.len() > eapol_start {  
                        let eapol = data_payload[eapol_start..].to_vec();
                        log::info!(  
            ...
            match cfg_subtype {  
                ...
                // 注意：已删除原来的 SDIO_TYPE_DATA 匹配臂（死代码）  
