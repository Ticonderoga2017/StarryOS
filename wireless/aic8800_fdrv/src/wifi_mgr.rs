...
pub fn connect(
    ...
    if wpa2_ie.is_empty() {  
        ...
        log::info!("[wifi_mgr] open network: control port opened for sta_idx={}", result.ap_idx);  
    } else {  
...
pub fn install_pairwise_key(
    ...
    log::info!(
        ...
        vif_idx, sta_idx, cipher, key.len()
    );
...
pub fn install_group_key(
    ...
    log::info!(  
        ...
        vif_idx, sta_idx, cipher, key_idx, key.len()  
    ); 
        ...
        Ok(ind) => {
            if ind.len() >= 4 {
                ...
                log::info!(  
                    ...
                    reason, vif, ft_over_ds  
                ); 
            }
        }
        Err(CmdError::Timeout) => {
            log::warn!("[wifi_mgr] SM_DISCONNECT_IND timeout (disconnect may still have succeeded)");  
        }
        Err(e) => {  
...
pub fn handle_disconnect_ind(param: &[u8]) {
    if param.len() >= 4 {
        ...
        log::warn!("[wifi_mgr] passive disconnect (short param: {} bytes)", param.len());
    }
