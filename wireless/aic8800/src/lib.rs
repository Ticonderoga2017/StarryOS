...
use fw_select::{FirmwareSet, select_firmware};
...
pub fn firmware_init<H: SdioHost>(
    ...
    // log::info!("[aic8800] firmware_init: chip={:?}, fw_size={}", chip, fw_set.wl_patch.len());
    ...
    // 5. 选择固件 
    let fw_set = select_firmware(chip, &chip_rev);
    let fw_set = match fw_set {
        Some(fw) => {
            log::info!(  
                "[aic8800] Firmware selected: {} (wl_fw={} bytes, patch_tbl={} bytes, wl_patch={} bytes)",  
                fw.desc,  
                fw.wl_fw.len(),  
                fw.patch_tbl.len(),  
                fw.wl_patch.len(),  
        ...
        None => {
            log::error!("[aic8800] No firmware available for this chip");  
            return Err(SdioError::Unsupported); 
        }
    };
